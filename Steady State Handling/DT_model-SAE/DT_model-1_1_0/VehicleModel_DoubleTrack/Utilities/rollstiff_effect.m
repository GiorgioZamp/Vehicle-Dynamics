function rollstiff_effect(roll_set,vehicle_data)

leg = cell(size(roll_set)); %initialize for speed
    datasets = cell(size(roll_set));

    for i=1:length(roll_set)
    ...
    exportgraphics(f,'Graphs/ToeEffect.eps')
end